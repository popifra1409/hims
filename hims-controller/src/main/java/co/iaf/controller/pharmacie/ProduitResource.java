...
public class ProduitResource {
	...
	// ajouter un nouveau produit (test Ok)
	...
	// afficher la liste des produits (test OK)
	...
	// recupérer un produit par son id (testok)
	...
	//recuperer un produit par son qrcode
	@GetMapping(path = "/produits/qrcode/{id}")
	public ResponseEntity<?> getProduitByQrCode(@PathVariable(name = "id") Long qrcode) {
		Map<String, Object> map = new LinkedHashMap<String, Object>();
		try {
			Produit produit = produitService.getByQrCodeProduit(qrcode);
			map.put("status", 1);
			map.put("data", produit);
			return new ResponseEntity<>(map, HttpStatus.OK);
		} catch (Exception ex) {
	...
	// Grouper les produit
	/*
	 * @PostMapping(path = "/produits/save/{ref1}/{ref2}") public ResponseEntity<?>
	 * saveGroupeProduit(@PathVariable(name = "ref1") String produitPrincipalId,
	 * 
	 * @PathVariable(name = "ref2") String produitSecondaireId, @RequestBody
	 * GroupeProduit groupeProduit) { Map<String, Object> map = new
	 * LinkedHashMap<String, Object>();
	 * produitService.addNewGroupeProduit(groupeProduit, produitPrincipalId,
	 * produitSecondaireId); map.put("status", 1); map.put("message", new
	 * ApiResponse("Produit groupé avec succès !", true)); return new
