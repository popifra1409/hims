...
public class FacturationResource {
	...
	public ResponseEntity<?> saveDomaine(@PathVariable(name = "domaineParentId") Long parentId,
			@PathVariable(name = "imputationId") Long imputationId, @RequestBody Prestation prestation) {
	...
	// ajouter une nouvelle Imputation
	@PostMapping(path = "/factures/save/{patientId}")
	public ResponseEntity<?> saveFacture(@PathVariable String patientId, @RequestBody Facture facture) {
		Map<String, Object> map = new LinkedHashMap<String, Object>();
		facturationService.addNewFacture(facture, patientId);
		map.put("status", 1);
		map.put("message", new ApiResponse("Facture créé avec succès !", true));
		return new ResponseEntity<>(map, HttpStatus.CREATED);
	}
}
